...
현재 로컬 실행 환경에서는 Poller 방식을 사용한다.
...
8. `pytest`, `unittest`, `ruff`, `compileall`과 Git Diff로 변경을 검증한다.
...
- 기본 모델: `gemini/gemini-3.6-flash`
- 예비 모델: `groq/openai/gpt-oss-120b`
- 공통 호출 계층: LiteLLM
- 단계 사이 최소 대기: 4.1초
- 기본 오류 재시도: 최대 2회
- Gemini 할당량 초과 시 해당 Issue의 남은 단계를 Groq로 전환한다.
...
- 명령 실행 시간은 최대 30초로 제한한다.
- CLI 출력은 최대 1,000자 또는 50줄로 제한한다.
- 하나 이상의 검증이 성공하지 않거나 검증이 실패하면 PR을 생성하지 않는다.
